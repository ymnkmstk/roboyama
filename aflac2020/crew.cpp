...
void Observer::operate() {
    ...
    distLogger.logging(getDistance());
    locXLogger.logging(getLocX());
    locYLogger.logging(getLocY());
    ...
    //if ((++traceCnt * PERIOD_OBS_TSK >= PERIOD_TRACE_MSG) && (d < 11000)) {
        ...
        _debug(syslog(LOG_NOTICE, "%08u, Observer::operate(): distance = %d, azimuth = %d, x = %d, y = %d", clock->now(), distLogger.lastValue(), getAzimuth(), locXLogger.lastValue(), locYLogger.lastValue()));
